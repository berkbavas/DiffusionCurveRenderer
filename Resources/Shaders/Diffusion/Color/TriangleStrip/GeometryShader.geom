...
void main()
{
    ...
    vec4 leftColorAt_t0 = leftColorAt(t0);
    vec4 leftColorAt_t1 = leftColorAt(t1);

    vec4 rightColorAt_t0 = rightColorAt(t0);
    vec4 rightColorAt_t1 = rightColorAt(t1);

    // Left side
    {
        gl_Position = projectionMatrix * vec4(valueAt_t0 - 0.5f * width * normalAt_t0, 0, 1);
        fs_Color = leftColorAt_t0;
        EmitVertex();

        gl_Position = projectionMatrix * vec4(valueAt_t0 - 1.0f * width * normalAt_t0, 0, 1);
        fs_Color =  leftColorAt_t0;
        EmitVertex();

        gl_Position = projectionMatrix * vec4(valueAt_t1 - 0.5f * width * normalAt_t1, 0, 1);
        fs_Color = leftColorAt_t1;
        EmitVertex();

        gl_Position = projectionMatrix * vec4(valueAt_t1 - 1.0f * width * normalAt_t1, 0, 1);
        fs_Color = leftColorAt_t1;
        EmitVertex();
